...
const assetController = new AssetController(getAssetsUseCase, getAssetDetailsUseCase, sharedUow);
...
const indexMediaUseCase = new IndexMediaUseCase(sharedUow, processingService, eventBus, SIDECARS_DIR);
...
const nsfwWorker = new NsfwTaskRunner(eventBus, sharedUow, NSFW_INTERVAL_MS, ollamaService, NSFW_THRESHOLD, OLLAMA_VISION_CONCURRENCY);
...
app.get('/api/assets/:id/media', async (req, res) => {
  if (!(await requireUser(req, res, getSessionUserUseCase))) return;
  await assetController.getAssetMedia(req, res);
});
