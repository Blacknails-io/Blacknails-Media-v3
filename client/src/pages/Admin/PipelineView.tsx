        ...
        <div className="import-panel-toolbar">
          ...
          <div className="import-panel-toolbar-actions">
            ...
            <button
              ...
              onClick={async () => {
                if (!confirm('¿Forzar reproceso global de todos los assets? (Puede tardar horas)')) return;
                try {
                  const res = await fetch('/api/assets/reprocess', { method: 'POST' });
                  if (!res.ok) throw new Error('Error al enviar orden de reproceso');
                  alert('Orden de reproceso enviada. Los workers indexadores se reactivarán.');
                } catch (e: any) {
                  alert(e.message);
                }
              }}
              title="Añadir a la cola todos los assets de nuevo"
            >
              Reprocesar Todo
            </button>
          </div>
