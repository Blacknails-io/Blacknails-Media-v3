...
export function usePipelineLogic() {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    try {
      const workers = await pipelineService.getWorkers();
  ...
  const handleAction = useCallback(async (workerId: string, action: WorkerAction) => {
    try {
      ...
      if (action === 'start') {
        updated = await pipelineService.startWorker(workerId);
      } else if (action === 'stop') {
        updated = await pipelineService.stopWorker(workerId);
      } else if (action === 'reset') {
        updated = await pipelineService.resetWorker(workerId);
      } else {
        updated = await pipelineService.triggerWorker(workerId);
      }
      setState((prev) => ({
        ...prev,
  ...
  const handleGroupAction = useCallback(async (
    ...
    const runAction = async (id: string): Promise<PipelineWorkerDTO> => {
      if (action === 'start') return pipelineService.startWorker(id);
      if (action === 'stop') return pipelineService.stopWorker(id);
      if (action === 'reset') return pipelineService.resetWorker(id);
      return pipelineService.triggerWorker(id);
    };
